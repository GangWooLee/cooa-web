...
class WorkspaceLifecycleTest < ActionDispatch::IntegrationTest
  ...
  test "4종 라벨 렌더 — 작업실 초대 폼 role select는 팀 4종(한글 라벨)만" do
    sign_in_as(@kim)
    ws = Product.find_by!(name: "비타민C 브라이트닝 앰플").workspace
    get workspace_path(ws)
    assert_response :success
    opts = css_select("form[action='#{invitations_path}'] select[name='role_key'] option").map { |o| o.text.strip }
    assert_equal [ "관리자", "멤버", "뷰어", "외부 협력" ], opts, "작업실 초대 폼 = 팀 4종 한글 라벨(전사 전용 역할 부재)"
  end
end
