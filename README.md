...
![데모: 도안 업로드부터 텍스트 추출·확정, 스크리닝, 피드백, 검토 확인까지](docs/images/demo.gif)

30초 데모입니다. 도안 PDF를 올리고, 텍스트를 추출해 사람이 확정하고, 국가별 스크리닝 판정을 받고, 영역 피드백과 검토 확인으로 이어집니다.
...
> 검토 엔진은 룰 선행입니다. `app/services/screening_service.rb`는 LLM 없이 동작하는 결정론적 룰엔진이고, 이것이 기본 경로입니다.
> 그 위에 JP 한정 LLM 보조판정 경로(`app/services/llm_screening_judge.rb`)가 구현되어 — API 키가 설정된 경우에만 — 경계 사례(판단불가·주의)를 규제 근거 인용과 함께 재판정합니다.
> 키가 없으면 순수 룰엔진으로 동작합니다. LLM 판정도 인용은 제공된 규제 fact에 바인딩되고(환각 인용은 자동 기각), 최종 검토 필요는 그대로 유지됩니다. 룰-only 대비 판정 차이의 실측 문서화는 실연동(키 설정) 후 진행합니다.
> 도안 파일에서 성분·라벨 문구를 뽑는 경로는 생겼지만 — 텍스트 레이어가 있는 PDF를 휴리스틱으로 추출해 사람이 확정하는 PoC이고(OCR은 없음) —
> 스크리닝은 그렇게 확정됐거나 시드된 성분·라벨을 규제 지식베이스와 대조합니다. 위반 영역 표시 좌표는 여전히 데모 도안에 맞춘 하드코딩입니다.
...
- 성능 벤치마크: 클릭에서 화면 반영까지의 인터랙션 레이턴시 실측과 측정 방법론을 [docs/benchmarks/interaction-latency-2026-07.md](docs/benchmarks/interaction-latency-2026-07.md)에 남겼습니다.
  서버 쿼리 최적화 사이클(요청 세트 기준 쿼리 325→275)과 짝을 이루는 브라우저단 계측입니다.
  DB 계층에서는 RLS 술어 유무의 오버헤드를 owner/cooa_app 두 연결로 실측해 [docs/benchmarks/rls-overhead-2026-07.md](docs/benchmarks/rls-overhead-2026-07.md)에 남겼습니다(절대 차이 1ms 미만·진짜 신호는 플랜 변형).

### 성능 전후 비교

서버 사이클은 대량 데모 상태(제품 255, 버전 2,516)에서 화면당 쿼리 수를 계측하고 병목 11건을 수술한 기록입니다.
...
스크리닝          29  ██████████████░               25  █████████████
리뷰 인박스       28  ██████████████                25  █████████████
구성원            27  █████████████░               28  ██████████████  (+1: 인증 프리로드 트레이드오프)
────────────────────────────────────────────────────────────────
8화면 합계      325 → 275 (-15%)  ·  트리 재귀 N+1 5→0  ·  배지 COUNT 2→1  ·  실N+1 3→0
```

인터랙션 사이클(클릭에서 화면 반영까지)의 원인 귀속은 다음과 같습니다.

| 판정 | 건수 | 원인 축 |
|---|---|---|
| 즉각 (100ms 미만) | 6 | 전부 순수 클라이언트 — 토글, 필터, 낙관 UI 0~16ms |
| 지연 (1초 미만) | 6 | 전부 서버 왕복 |
| 문제 (1초 초과) | 4 | 서버, PDF 자산, 의도된 연출 — 브라우저 원인 0건 |

수치는 dev 모드 기준으로 절대값이 아니라 상대 비교가 신호입니다. 같은 화면이 세션 부하에 따라
최대 55배 요동함을 교차 검증으로 확인했고, 방법론 전체는 [docs/benchmarks/](docs/benchmarks/interaction-latency-2026-07.md)에 있습니다.
...
| 팀의 작업실을 한눈에 보고 바로 진입합니다 | 제품과 도안 버전이 어디까지 왔는지 폴더 트리에서 파악합니다 |
| ![버전 상세](docs/images/03-version.png) | ![스크리닝 결과](docs/images/04-screening.png) |
| 도안 위에 영역을 짚어 피드백을 남기고 반영 여부를 추적합니다 | 국가별 규제 위반 후보를 도안 위 좌표와 근거 조항으로 받습니다 |
| ![리뷰 패널](docs/images/05-review.png) | ![버전 비교](docs/images/06-compare.png) |
| 검토를 요청하고 확인하며, 그 기록이 감사 체인에 남습니다 | 버전 사이 달라진 점과 피드백 반영 여부를 나란히 확인합니다 |
...
- 가장 큰 한계는 이름에 걸린 검토 엔진이 아직 룰 매칭이라는 점입니다. 도안에서 텍스트를 뽑아 사람이 확정하고
  스크리닝으로 잇는 파이프라인은 이제 동작하지만(텍스트 레이어 PDF 한정·휴리스틱·OCR 없음), LLM 하이브리드 판정이
  아직 없어 이 제품의 핵심 가설은 검증되지 않은 상태로 남습니다. 아래 로드맵의 1순위입니다.

## 로드맵

배포 운영 대신 다음 고도화에 우선순위를 둡니다.

1. 검토 엔진 최소판 (부분 완료 · 하이브리드 경로 구현 · 실연동 검증 대기): 한 국가(일본)부터 규제 지식베이스와 LLM을 결합한 하이브리드 판정을 구현했습니다. 룰 선행 + 경계 사례만 LLM 재판정 + 인용 fact 바인딩. API 키 설정 시 활성화되며, 실측 문서화가 남아 있습니다. 코드: `app/services/llm_screening_judge.rb`.
2. 도안 텍스트 추출 (완료 · PoC): 업로드된 PDF의 텍스트 레이어에서 성분표와 라벨 문안을 휴리스틱으로 추출하고 사람이 확정해, 스크리닝 입력을 시드가 아니라 실제 파일로 잇습니다(OCR·LLM 판정은 로드맵 1번). 코드: `app/services/dieline_text_parser.rb` · `pdf_text_extractor.rb` · `app/controllers/label_extractions_controller.rb`.
3. 성능 문서화 (완료): 인터랙션 레이턴시 실측과 RLS 술어 유무의 DB 계층 오버헤드 실측을 [docs/benchmarks/](docs/benchmarks/rls-overhead-2026-07.md)에 공개했습니다.

## English summary

COOA is a regulatory pre-screening tool and review workspace for cosmetics packaging artwork. It is not a design editor. Teams draw artwork in Figma or Illustrator; COOA reviews it, judges it against regulatory rules, and keeps the record.

What is real and running: a collaboration workspace with tenant isolation enforced at the database. Row-level security guards 20 tables and fails closed, so a missing tenant context returns nothing rather than leaking rows. The audit log is a hash chain, and the authorization tests are derived from a permission matrix rather than written by hand. Uploaded PDFs go through an extraction path — the text layer is parsed heuristically, then a human confirms it — before feeding the screening engine.

What is honest about its limits: the screening engine is rule-first and deterministic. A JP-only LLM-assist path re-judges boundary cases (undecided or needs-review), but it is key-gated — without an API key the engine runs pure rules, and LLM verdicts must cite from the supplied regulatory facts or be discarded. There is no OCR; extraction relies on an existing PDF text layer. The bounding-box coordinates that highlight violations on the artwork are still hardcoded to the demo dieline. Knowing that boundary is the starting point for reading this repository.
