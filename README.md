...
> AI 검토 엔진은 아직 만들지 않았습니다. 현재 `app/services/screening_service.rb`는 LLM 없이 동작하는 결정론적 룰엔진입니다.
> 도안 파일에서 성분·라벨 문구를 뽑는 경로는 생겼지만 — 텍스트 레이어가 있는 PDF를 휴리스틱으로 추출해 사람이 확정하는 PoC이고(OCR도, LLM 판정도 없음) —
...
3. 성능 문서화: 인터랙션 레이턴시 실측은 [docs/benchmarks/](docs/benchmarks/interaction-latency-2026-07.md)에 공개했습니다. RLS 오버헤드 측정이 남아 있습니다.
