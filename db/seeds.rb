...
[ ApprovalStep, ApprovalRequestReviewer, ApprovalRequest, # 리뷰: ScreeningRun보다 먼저(approval_request→run FK)
 ...
 ComponentVersion, Component, ProductMember, ProductProperty, Product, Workspace, # Product→Workspace(products.workspace_id RESTRICT)
...
# 6번째 페르소나 — 정브랜(브랜드관리). "비타민C 브라이트닝 앰플" 작업실에 brand_admin **workspace-scope**
# = 그 작업실의 팀 admin(scoped sub-admin, WS-track). tenant-wide 아님 → 자기 작업실만 관리·가시.
