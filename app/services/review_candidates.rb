...
# 가진 계정 ∪ tenant-wide grant 보유 계정의, 연결된 도메인 User(user_id 있는 것만). 단 external_collaborator뿐인
# 계정은 후보에서 제외한다(users_for 내 근거 주석 — 검토 확인 무결성).
#
...
module ReviewCandidates
  ...
  # 후보 계정의 연결 User 배열(user_id 있는 것만). exclude_user_id로 제출자/뷰어 1인 제외. N+1 없음
  # (grants pluck 1 + users 1 = 상수 2쿼리). resolver/product_policy와 동일한 "product grant = 서브트리"
  # 의미를 재사용(브랜드 루트의 self_and_descendant_ids · 그 서브트리 컴포넌트).
  ...
  # 화이트리스트 소스 — 후보 User id 배열(제출자 strip은 모델 sync_requested_reviewers!가 담당).
