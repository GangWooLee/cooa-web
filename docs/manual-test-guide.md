...
### ③ 외부 협력자 작업실 온보딩 — 작업실 초대 → 시크릿창 수락 → 제한 뷰 → 두 번째 작업실 추가 → 회수
1. **[김쿠아·owner]** 사이드바에서 작업실(비타민C 브라이트닝 앰플) 진입 → 헤더 **"멤버 초대·관리"** 펼치기 → 이메일·역할 `external_collaborator`로 **이 작업실에 초대**(범위 선택 없음 — 서버가 이 작업실로 고정) → **링크 복사** → *이 작업실 pending 초대에 등재* (§4.6)
2. **[시크릿 창]** 링크 열기 → "**비타민C 브라이트닝 앰플** 작업실에 참여합니다" → Google로 수락 → *홈에 **그 작업실 카드만** · 타 작업실(레티놀·시카) 부재* (§5.1)
3. **[외부 협력자]** 타 작업실 제품 URL 직접 진입 → *권한 안내 후 루트로 리다이렉트(콘텐츠 미노출)* (§5.2)
4. **[김쿠아]** 두 번째 작업실(시카 수딩 크림) 진입 → 헤더 **"멤버 초대·관리"** → **"기존 멤버 추가"**로 그 협력자 선택·추가 → *협력자는 재로그인 없이 즉시 두 작업실 가시* (§5·§5.4)
5. **[김쿠아]** 그 작업실 패널의 현재 멤버 배지 옆 **×**로 회수 → *협력자의 다음 요청에서 그 작업실 소멸(fail-closed·요청 간 stale 역할 없음)* (§5). 전사 관리(`/members`)는 이제 **읽기 위주 계정 대장** — 인원 추가·회수는 각 작업실 페이지에서(D5).

### ④ 작업실 수명주기 — 생성(이름+멤버 4종) → 빈 상태 → 첫 폴더 → 이름변경 → 삭제(빈 작업실만)
1. **[김쿠아·owner]** 홈(`/`) 우상단 **"새 작업실"** → 모달에서 이름(예: "2026 리뉴얼 TF") + (선택) 멤버 체크·역할(관리자/멤버/뷰어/외부 협력 **4종**) → **"작업실 만들기"** → *그 작업실로 진입, "아직 폴더나 항목이 없습니다" 빈 상태* (§8.4)
2. **[김쿠아]** 툴바 **새 폴더/새 파일**로 첫 트리 생성 → *이 작업실에 귀속(복수 루트 수용 — 한 작업실에 여러 루트 트리)* (§8.4)
3. **[김쿠아]** 홈 복귀 → 카드 **우클릭** → **이름 변경**(prompt) · **삭제**(빈 작업실만 — 제품이 남아 있으면 "먼저 비우세요" 안내). *생성·이름변경·삭제는 tenant-wide 관리자만* (§8.4)

### ⑤ 작업실 경계 — 리소스 진입 후 되돌아오기(컨텍스트↔본문 정합)
1. **[김쿠아·owner]** 사이드바에서 작업실(레티놀 3% 세럼) 진입 → 리프(일본/CO0001) 열기 → 구성요소 **버전 칩** 클릭 → *풀페이지 버전 뷰* (§1-4)
2. **[김쿠아]** 버전 뷰 브레드크럼의 **제품(일본) 링크** 클릭 → *제품 상세로 풀 복귀 — 본문 트리·사이드바 **모두 여전히 레티놀 작업실만**(타 작업실 비타민C·시카 행 부재)* (§8.5). 대조로 홈/인박스/전사 관리로 가면 컨텍스트 해제(작업실 밖).
...
## ⑧ 작업실 컨텍스트 경계 (W1/W2/W3)

### ☐ 8.1 홈 = 작업실 카드 · 진입 시 컨텍스트 전환 `[UI]`
- **무엇** — 홈(`/`)은 트리 테이블이 아니라 **작업실 카드 목록**. 카드 클릭 → 그 작업실(`/brands/:id`) 진입 → 좌측 사이드바가 그 작업실 서브트리 트리로 전환(Figma식 컨텍스트 전환). **"← 모든 작업실"**로 홈 복귀.
- **확인** — 카드에 작업실명·제품 수·멤버 아바타. 진입 후 사이드바 헤더=작업실명, 트리=그 서브트리만(복수 루트 수용). 인박스·전사 관리는 컨텍스트 비의존(작업실 밖). **"새 작업실"**(tenant-wide 관리자만)은 모달로 이름+멤버(4종)를 받아 **빈 작업실**을 만들고 그 작업실로 진입(§8.4).
- **메커니즘** — `current_workspace`(렌더 시점 lazy — `@workspace`(작업실 진입) 또는 `@product`(리소스)에서**만** 도출 · 리소스 없는 글로벌 화면은 항상 해제) 단일 출처로 홈 카드·사이드바가 갈린다. `dashboard#index`가 카드/트리 분기. 세션에 직전 작업실을 저장·복원하지 않는다(F1: 되살리면 본문↔사이드바 분열 — §8.5).

### ☐ 8.2 스코프 계정은 자기 작업실만 `[UI · 전환]`
- **무엇** — 최디자(external @ CO0200)로 전환 → 홈에 **자기 작업실 카드 1개만**, 타 작업실·비가시 조상 브랜드 부재(재루팅).
- **메커니즘** — `ProductPolicy::Scope`(가시집합)이 홈 카드·사이드바 트리·배지 클립의 단일 출처. 부모가 비가시면 그 노드가 표시 루트로 승격(브랜드명 유출 차단).

### ☐ 8.3 작업실 초대는 서버가 그 작업실로 강제 + 역할 4종 화이트리스트 `[UI · devtools]`
- **무엇** — 작업실 페이지 **"멤버 초대·관리"**의 초대 폼엔 범위 선택이 없다(hidden `scope_workspace_id`=이 작업실). devtools로 그 hidden을 **타 작업실 id로 위조**해도 → **403·미생성**. 더해 **역할도 팀 4종만**(관리자/멤버/뷰어/외부 협력) — `role_key`를 전사 전용 역할(승인자/검토자 등)로 위조하면 **R9 안내·미생성**(D4). 복귀 경로(`return_to_workspace`)는 표시용일 뿐 경계가 아니다.
- **메커니즘** — §2.2와 동형(직접 POST 우회 방어). 2단 인가 `authorize_member_write!(대상=작업실 대표 루트 제품)`가 관할 밖이면 자연 deny + `scoped_role_permitted?`(4종 화이트리스트)가 역할 위조 차단. 전사 초대(모든 작업실·7종)는 전사 관리에서 tenant-wide admin만.

### ☐ 8.4 작업실 CRUD — 생성·이름변경·삭제(빈 작업실만) `[UI]` — D3
- **무엇** — 작업실은 first-class 컨테이너(복수 루트 수용). 홈 **"새 작업실"** 모달(이름 + 선택 멤버 4종)로 만들고, 홈 카드 **우클릭**으로 이름 변경(prompt→PATCH)·삭제(confirm→DELETE). 삭제는 **빈 작업실만**(제품이 남아 있으면 R9 안내 — DB FK RESTRICT 백스톱).
- **확인** — 생성 후 그 작업실 빈 상태 진입("아직 폴더나 항목이 없습니다"). 툴바 새 폴더/새 파일은 **이 작업실**에 귀속(heal로 새 작업실 만들지 않음). 제품 있는 작업실 삭제 시도 → "제품이 남아 있는 작업실은 삭제할 수 없습니다". 생성·이름변경·삭제는 tenant-wide 관리자(owner/brand_admin tenant-wide)만 — scoped admin(정브랜)·비관리자(박)는 **403**.
- **메커니즘** — `WorkspacesController`(create/update/destroy) — `authorize current_organization, :manage_product?`(tenant-wide 게이트, WorkspacePolicy 신설 없이 record-독립 역할 해석) + 감사(`workspace.create`/`rename`/`destroy`). 삭제는 `Workspace has_many :products, dependent: :restrict_with_exception` + FK RESTRICT. 생성 시 선택 멤버 = workspace-scope role_assignment(4종 화이트리스트·SAVEPOINT 격리). 빈 작업실 가시성은 `directly_visible_workspaces`(tenant-wide=전 작업실·scoped=workspace grant)가 보완. `test/integration/workspace_lifecycle_test.rb`·`test/system/workspace_lifecycle_test.rb`.

### ☐ 8.5 리소스→컨텍스트 복귀 정합 (되돌아오기) `[UI]` — F1
- **무엇** — 작업실(레티놀 3% 세럼) 진입 → 리프(일본/CO0001) 열기 → 구성요소 **버전 칩**으로 **풀페이지 버전 뷰** 이동 → 버전 뷰 브레드크럼의 **제품(일본) 링크** 클릭으로 제품 상세에 **풀 복귀**. 복귀 후 본문 트리 테이블과 사이드바가 **모두 레티놀 작업실만** 표시해야 한다.
- **재현(픽스 전 증상)** — 픽스 전엔 리프 상세 풀요청이 본문 셸을 **전 작업실**로 렌더해, 사이드바(그 작업실)와 본문(전 작업실)이 분열했다(리소스↔컨텍스트 불일치). 복귀 트리거 = 버전 뷰 브레드크럼의 리프 세그먼트(풀 내비 `product_path`).
- **확인** — 복귀한 제품 상세에서 본문 `main` 트리 행 = 레티놀 서브트리(레티놀 3% 세럼·미국·30ml/CO0000·일본/CO0001)만 · **비타민C/시카 루트 부재**. 사이드바 헤더=레티놀 + "← 모든 작업실" 백링크. 대조: 홈(`/`)·인박스(`/reviews`)·전사 관리(`/members`)는 리소스가 없으므로 **항상 컨텍스트 해제**(작업실 목록 사이드바 — §8.1).
- **메커니즘** — `products#show` 풀요청이 `load_dashboard_rows(workspace: workspace_of_node(@product))`로 셸 트리를 그 리프의 작업실로 스코프(`dashboard#index`의 focus 경로와 동일 계약). `current_workspace`도 `@product`에서 같은 작업실을 도출 → 본문↔사이드바 정합. 세션 폴백 없음. `test/integration/dashboard_body_scope_test.rb`(본문 셀렉터=`main` 트리 vs 사이드바 구분)·`test/system/workspace_nav_test.rb`(브레드크럼 복귀 저니)·`test/integration/dashboard_tree_test.rb`(본문 스코프+N+1).
...
| **작업실 경계 (Context)** | ⑧ 8.1~8.5 / ③④⑤ | 홈=작업실 카드·컨텍스트 전환·스코프 자기 작업실만·작업실 초대/역할(4종) 서버 강제·작업실 CRUD(빈 삭제만)·**리소스 복귀 후 본문↔사이드바 정합**(8.5) |
...
멤버 로스터에 해당 계정이 **`외부 협력 · <작업실명>`(한글 라벨·읽기전용 링크) 배지**로 등재(D4/D5). 회수·추가는
각 **작업실 페이지**의 멤버 패널("멤버 초대·관리")에서 — 전사 관리(`/members`)는 읽기 위주 계정 대장(D5).
audit_logs에 `invitation.create`(scope 포함)·`invitation.accept`·`role_assignment.grant`/`revoke` 기록.
...
>
> 📌 **이메일당 pending 초대는 1건**(부분 유니크). 한 계정을 **여러 작업실/제품**에 스코프하려면 초대 수락 후
> **직접 grant**로 추가한다 — 각 **작업실 페이지**의 "기존 멤버 추가" 폼 또는 §5.4 콘솔 레시피.

### ☐ 5.0 역할 라벨 · 두 기구(작업실 멤버 vs 제품 한정 게스트) `[참고]` — D4/D5
- **역할 라벨**(`app/policies/authz/role_labels.rb`) — 화면은 한글 라벨, 저장·감사·매트릭스는 role_key 원문(불변):

  | role_key | 라벨 | 1줄 설명 | 작업실 폼 |
  |---|---|---|---|
  | brand_admin | 관리자 | 작업실·멤버 관리 | ✓ |
  | contributor | 멤버 | 업로드·피드백·리뷰 요청 | ✓ |
  | viewer | 뷰어 | 열람만 | ✓ |
  | external_collaborator | 외부 협력 | 업로드·피드백, 승인 불가 | ✓ |
  | ra_reviewer | 검토자 | 검토(RA) | ✗(전사 전용) |
  | approver | 승인자 | 승인 | ✗(전사 전용) |
  | assignee | 담당 편집자 | 담당 편집 | ✗(전사 전용) |
  | owner | 소유자 | 조직 전체 관리 | ✗(초대 불가) |

- **두 기구**:
  - **작업실 멤버**(workspace-scope) — 그 작업실의 **모든 루트 서브트리** 가시. 작업실 폼(초대·기존 멤버 추가·생성 시 멤버)은 **팀 4종만**(관리자/멤버/뷰어/외부 협력). 서버가 4종 화이트리스트로 강제 — 전사 전용 역할(승인자/검토자 등)을 작업실 경로로 위조 발급 시도하면 **R9 안내·미생성**(§8.4).
  - **제품 한정 게스트**(product/component-scope) — 외부 에이전시처럼 **제품 하나(서브트리)만**. 초대·직접 grant 양 경로가 테넌트 소속 검증을 공유(§4.6·§5.1~5.4).
  - **전사 멤버**(tenant-wide) — 모든 작업실 접근. 전사 관리(`/members`) 초대 폼은 **7종**(owner 제외 전 역할·한글 라벨).
...
- 제품·작업실 0 테넌트 대시보드 → 빈 상태 우아 렌더(빈 작업실은 카드로 표시 — D3) `[자동:edge_empty_state_test.rb]`
- 빈 작업실 진입 → 트리 빈 상태("아직 폴더나 항목이 없습니다") + 툴바 첫 항목 유도 `[자동:test/system/workspace_lifecycle_test.rb]`
...
### (m) 작업실 수명주기 · 역할 라벨 (D3/D4/D5)
- 생성(이름+멤버 4종)→빈 상태→제품 2트리→workspace grant 멤버 접근·비멤버 차단→이름변경→삭제(빈 작업실만·RESTRICT) `[자동:test/integration/workspace_lifecycle_test.rb]`
- 작업실 CRUD는 tenant-wide 관리자만 — scoped admin(정브랜)·비관리자(박)는 생성 403 `[자동:workspace_lifecycle_test.rb]`
- 역할 위조 차단 — 작업실/제품 경로 발급은 팀 4종만(전사 전용 역할 approver·ra_reviewer는 R9·미생성) `[자동:workspace_lifecycle_test.rb]`
- 4종 한글 라벨 렌더(작업실 폼) · 전사 폼 7종 · 로스터 배지 라벨+작업실 링크 `[자동:workspace_lifecycle_test.rb · test/system/members_scope_test.rb]`
- 홈 모달 생성(이름+멤버) + 빈 상태 + 툴바 첫 폴더(작업실 귀속) + 카드 우클릭 이름변경 `[자동:test/system/workspace_lifecycle_test.rb]`
- 복수 루트 작업실(한 작업실 2루트) — 멤버 두 루트 가시·비멤버 차단·멤버 요약 `[자동:test/integration/multi_root_workspace_test.rb]`
