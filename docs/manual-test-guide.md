...
- **메커니즘** — `current_workspace`(렌더 시점 lazy — `@brand`/`@product`에서 도출 · 폴백 `session[:workspace_id]`) 단일 출처로 홈 카드·사이드바가 갈린다. `dashboard#index`가 카드/트리 분기.
...
| **작업실 경계 (Context)** | ⑧ 8.1~8.4 / ③④ | 홈=작업실 카드·컨텍스트 전환·스코프 자기 작업실만·작업실 초대/역할(4종) 서버 강제·작업실 CRUD(빈 삭제만) |
