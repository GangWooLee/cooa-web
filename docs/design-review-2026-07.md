...
> **[2026-07-10 적용 현황]** WP1~WP9 전 묶음 적용 완료 (feat/design-foundation, 원자 커밋 19개 — DESIGN.md 성문화 b1c972c부터 a11y 마감 6f59a79까지). 유닛 425런·시스템 배치 그린.
> **잔여(후속 백로그)**: ①모달 크롬 partial 추출(2곳 중복 — §4 표준만 명문화됨) ②트리 테이블 모바일 풀 카드화(현재는 가로 스크롤+페이드 단서까지) ③전역/교차 작업실 검색(제품 결정 필요 — ia-nav-5) ④'디자인' 카테고리 캔버스 색 ink 수렴의 구분성 관찰(#4f74e3 파랑 제거됨) ⑤드로어 담당자·커스텀 속성의 'None' 표기(테스트 selector 결속) ⑥온보딩·조직선택·초대 3화면 캡처/리뷰(OAuth 중간 상태라 이번 기준선에서 제외됨).
